...
test('normalize text element rect after change its font', async ({ page }) => {
  ...
  await assertEdgelessSelectedRect(page, [130, 160, 102.8, 152]);
  ...
  await assertEdgelessSelectedRect(page, [130, 160, 112.5, 134]);

  await fontButton.click();
  const scibbledTextFont = page.getByText('Scibbled');
  await scibbledTextFont.click();
  await assertEdgelessSelectedRect(page, [130, 160, 100.7, 152]);
});
