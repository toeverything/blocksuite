...
export async function getNoteRect(page: Page, noteId: string) {
  const xywh: null | string = await page.evaluate(
    ([noteId]) => {
...
type EdgelessTool =
  ...
  | 'shape'
  | 'text';
type ZoomToolType = 'fitToScreen' | 'zoomIn' | 'zoomOut';
type ComponentToolType = 'brush' | 'more' | 'shape' | 'thick' | 'thin';
