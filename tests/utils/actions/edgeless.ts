...
export function locatorEdgelessComponentToolButton(
  ...
  const button = page
    .locator('edgeless-element-toolbar-widget affine-icon-button')
    .filter({
      hasText: text,
    });
...
export async function openComponentToolbarMoreMenu(page: Page) {
  const btn = page.locator(
    'edgeless-element-toolbar-widget edgeless-more-button affine-menu-button'
  );
  ...
  switch (action) {
    case 'bringToFront': {
      ...
      const actionButton = moreButton
        .locator('.more-actions-container affine-menu-action')
        .filter({
