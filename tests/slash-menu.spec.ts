...
test.describe('slash search', () => {
  ...
  test('slash menu supports fuzzy search', async ({ page }) => {
    ...
    await type(page, 'c');
    await expect(slashItems).toHaveCount(6);
    await expect(slashItems.nth(0).locator('.text')).toHaveText(['Code Block']);
    ...
    await expect(slashItems.nth(3).locator('.text')).toHaveText(['Attachment']);
    await expect(slashItems.nth(4).locator('.text')).toHaveText(['Copy']);
    await expect(slashItems.nth(5).locator('.text')).toHaveText(['Duplicate']);
    await type(page, 'b');
    await expect(slashItems.nth(0).locator('.text')).toHaveText(['Code Block']);
  });
