...
import { prepareClipboardData } from '../../../controllers/clipboard.js';
import type {
  EdgelessBlockModel,
  EdgelessModel,
  HitTestOptions,
...
export class DefaultTool extends ToolController {
  get name() {
    return 'default';
  }
  ...
  private _onClick(e: PointerEventState) {
    const selected = this._pick(e.x, e.y);
  ...
  private _onDblClick(e: PointerEventState) {
  ...
  private async _onDragStart(e: PointerEventState) {
    ...
    // Set up drag state
    this._initializeDragState(e, dragType);
  }

  private _onDragMove(e: PointerEventState) {
  ...
  override mount(): void {
    this.on('click', e => {
      this._onClick(e.get('pointerState'));
    });

    this.on('doubleClick', e => {
      this._onDblClick(e.get('pointerState'));
    });

    this.on('dragStart', e => {
      this._onDragStart(e.get('pointerState')).catch(console.error);
    });

    this.on('dragMove', e => {
      this._onDragMove(e.get('pointerState'));
    });
