...
> {
  ...
  private _buildPreviewSpec = (name: 'edgeless:preview' | 'page:preview') => {
  ...
  convertToCard = () => {
    const { caption, doc, id, pageId, xywh } = this.model;
    ...
    if (this.isInSurface) {
      ...
      const newId = edgeless.service.addBlock(
        ...
        { caption, pageId, style, xywh: bound.serialize() },
  ...
  covertToInline = () => {
    ...
    yText.format(0, REFERENCE_NODE.length, {
      reference: { pageId, type: 'LinkedPage' },
    });
