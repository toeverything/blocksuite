...
export type SerializedMindmapElement = {
  children: Record<string, NodeDetail>;
} & SerializedElement;

type MindmapElementProps = {
...
export class MindmapElementModel extends SurfaceGroupLikeModel<MindmapElementProps> {
  ...
  addNode(
     ...
     */
    parent: MindmapNode | null | string,
   ...
   */
  detach(subtree: MindmapNode | string) {
    subtree =
  ...
  getLayoutDir(node: MindmapNode | string): LayoutType | null {
   ...
   * ```
   */
  getPath(element: MindmapNode | string) {
