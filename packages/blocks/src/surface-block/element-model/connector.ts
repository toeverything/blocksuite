...
export type PointStyle = 'Arrow' | 'Circle' | 'Diamond' | 'None' | 'Triangle';
...
export type SerializedConnectorElement = {
  source: SerializedConnection;
  target: SerializedConnection;
} & SerializedElement;

export type ConnectorElementProps = {
...
export class ConnectorElementModel extends SurfaceElementModel<ConnectorElementProps> {
  ...
  override getNearestPoint(point: IVec2) {
    const { absolutePath: path, mode } = this;
  ...
  override hitTest(
    ...
    const { absolutePath: path, mode, strokeWidth } = this;
