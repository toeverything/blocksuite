...
export class Bound implements IBound {
  ...
  static fromDOMRect({ height, left, top, width }: DOMRect) {
    return new Bound(left, top, width, height);
  }

  static fromPoints(points: IVec[]) {
    const { maxX, maxY, minX, minY } = getBoundsFromPoints(points);
...
export function getBoundFromPoints(points: number[][]) {
  const { height, minX, minY, width } = getBoundsFromPoints(points);
