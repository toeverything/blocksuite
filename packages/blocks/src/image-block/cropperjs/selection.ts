...
export default class CropperSelection extends CropperElement {
  static override $name = CROPPER_SELECTION;
...
`;
    ...
    switch (action) {
        ...
        if (
          ...
          (this.dynamic ||
            (this.$actionStartTarget &&
              this.contains(this.$actionStartTarget as Node)))
        ) {
      ...
      case ACTION_SCALE:
        if (
          relatedEvent &&
          this.zoomable &&
          (this.dynamic || this.contains(relatedEvent.target as Node))
        ) {
  ...
  protected $handleKeyDown(event: Event): void {
    ...
    if (
      ...
      (['INPUT', 'TEXTAREA'].includes(activeElement.tagName) ||
        ['plaintext-only', 'true'].includes(
          (activeElement as HTMLElement).contentEditable
        ))
    ) {
      return;
    }
  ...
  protected $initSelection(center = false, resize = false) {
    ...
    if (isPositiveNumber(initialCoverage) && parentElement) {
      ...
      let width =
        (resize ? 0 : this.width) ||
        parentElement.offsetWidth * initialCoverage;
      let height =
        (resize ? 0 : this.height) ||
        parentElement.offsetHeight * initialCoverage;
