...
> {
  ...
  private _calculatePreviewOffset = (
    ...
    const { left, top } = blockElements[0].getBoundingClientRect();
  ...
  private _calculateSelector = (selectedIds: string[]): BlockSelector => {
    ...
    selectedIds.map(block => {
      ...
      do {
        if (!selectedIds.includes(parent)) {
          ids.push({ id: parent, type: BlockViewType.Bypass });
        }
    ...
    const addChildren = (id: string) => {
      ...
      children.forEach(child => {
        ids.push({ id: child.id, type: BlockViewType.Display });
  ...
  private _getDraggingAreaRect = (blockElement: BlockElement): Rect => {
    ...
    let { bottom, left, right, top } = blockElement.getBoundingClientRect();
  ...
  private _lastShowedBlock: { el: BlockElement; path: string } | null = null;
  ...
  private _updateDropResult = (dropResult: DropResult | null) => {
    ...
    if (dropResult?.rect) {
      ...
      let { height, width } = dropResult.rect;
