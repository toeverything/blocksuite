...
export type OnSuccessHandler = (
  pageIds: string[],
  options: { importedCount: number; isWorkspaceFile: boolean }
) => void;
...
export async function importNotion(collection: DocCollection, file: File) {
  ...
  const parseZipFile = async (file: Blob | File) => {
    const zip = new JSZip();
  ...
  return { hasMarkdown, isWorkspaceFile, pageIds };
}
...
export class ImportDoc extends WithDisposable(LitElement) {
  ...
  private async _importNotion() {
    ...
    const { hasMarkdown, isWorkspaceFile, pageIds } = await importNotion(
      this.collection,
      file
    );
    needLoading && this.abortController.abort();
