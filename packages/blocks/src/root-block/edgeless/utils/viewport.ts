...
export type ZoomAction = 'fit' | 'in' | 'out' | 'reset';
...
export class Viewport {
  ...
  viewportUpdated = new Slot<{ center: IVec2; zoom: number }>();
  ...
  onResize() {
    const { centerX, centerY, height: oldHeight, width: oldWidth, zoom } = this;
    const { left, top } = this._el.getBoundingClientRect();
    const { offsetHeight: height, offsetWidth: width } = this._el;
  ...
  toModelCoord(viewX: number, viewY: number): IVec2 {
    const { scale, viewportX, viewportY, zoom } = this;
  ...
  get viewportBounds() {
    const { viewportMaxXY, viewportMinXY } = this;
  ...
  get viewportMaxXY() {
    const { centerX, centerY, height, width, zoom } = this;
    return {
