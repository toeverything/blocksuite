...
export class EdgelessSnapManager extends Overlay {
  ...
  private _alignDistributeHorizontally(
    ...
    const wBoxes: Bound[] = [];
    this._referenceBounds.forEach(box => {
  ...
  align(bound: Bound): { dx: number; dy: number } {
    ...
    for (const other of this._referenceBounds) {
  ...
  setupSnapState(
    draggedElms: BlockSuite.EdgelessModel[],
    exclusion: BlockSuite.EdgelessModel[] = []
  ): Bound {
    if (draggedElms.length === 0) return new Bound();

    const connectors = draggedElms.filter(isConnectable).reduce((prev, el) => {
    ...
    const excludes = new Set([...draggedElms, ...exclusion, ...connectors]);
  ...
  updateReferenceBound(): Bound {
    ...
    (
      [...layer.blocks, ...layer.canvasElements] as BlockSuite.EdgelessModel[]
    ).forEach(alignable => {
      const bound = alignable.elementBound;
      if (
        viewportBounds.isOverlapWithBound(bound) &&
        !excludes.has(alignable)
      ) {
        this._referenceBounds.push(bound);
      }
    });
