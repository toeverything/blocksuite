...
type AbstractClassConstructor<T = object> = {
  prototype: T;
} & Constructor<T>;
...
export class EdgelessToolsManager {
  ...
  private _getToolFromLocalStorage(): EdgelessTool {
    ...
    if (type === 'pan') return { panning: false, type: 'pan' };
    return { type: 'default' };
  }
  ...
  get draggingArea() {
    ...
    const { end, start } = this.currentController.draggingArea;
