...
export class DefaultTool extends BaseTool {
  ...
  private _updateSelectingState = () => {
    ...
    if (gfx.keyboard.spaceKey$.peek()) {
      ...
      const [startX, startY] = gfx.viewport.toModelCoord(
    ...
    const [curX, curY] = gfx.viewport.toModelCoord(
      this._dragLastPos[0],
      this._dragLastPos[1]
    );
  ...
  private initializeDragState(
    ...
    const [startX, startY] = this.gfx.viewport.toModelCoord(x, y);
  ...
  override onload() {
    this.disposable.add(
      effect(() => {
        const pressed = this.controller.dragging$;
        ...
        if (pressed) {
          ...
          this._dragStartPos = this.gfx.viewport.toViewCoord(mX, mY);
