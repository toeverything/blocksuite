...
export class ShapeTool extends BaseTool<ShapeToolOption> {
  ...
  private _resize(shiftPressed = false, spacePressed = false) {
    ...
    let { startX, startY, endX, endY } = draggingArea;
    ...
    if (spacePressed && this._spacePressedCtx) {
      const lastMousePos = controller.lastMousePos$.peek();
      const pressedPos = this._spacePressedCtx.pressedPos;
      const pressedArea = this._spacePressedCtx.draggingArea;
      const [pressedX, pressedY] = viewport.toModelCoord(
        pressedPos.x,
        pressedPos.y
      );
      const [currentX, currentY] = viewport.toModelCoord(
        lastMousePos.x,
        lastMousePos.y
      );

      const deltaX = currentX - pressedX;
      const deltaY = currentY - pressedY;

      startX = pressedArea.startX + deltaX;
      endX = pressedArea.endX + deltaX;
      startY = pressedArea.startY + deltaY;
      endY = pressedArea.endY + deltaY;
    }
      ...
      effect(() => {
        ...
        if (spacePressed && this._draggingElementId) {
          this._spacePressedCtx = {
            ...
            pressedPos: this.controller.lastMousePos$.peek(),
          };
        }
      })
    );
