...
export class EdgelessDefaultToolButton extends QuickToolMixin(
  ...
  private _changeTool = () => {
    ...
    if (tool.type !== 'lasso') {
      this.setEdgelessTool({ mode: this.curMode, type: 'lasso' });
      return;
    }
    ...
    setTimeout(() => {
      this.curMode === LassoMode.FreeHand
        ? this.setEdgelessTool({ mode: LassoMode.Polygonal, type: 'lasso' })
        : this.setEdgelessTool({ mode: LassoMode.FreeHand, type: 'lasso' });
      this._fadeIn();
    }, 100);
  };
