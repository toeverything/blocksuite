...
export class EdgelessAutoCompletePanel extends WithDisposable(LitElement) {
  ...
  private _addNote() {
    ...
    const background = getComputedStyle(this.edgeless).getPropertyValue(color)
      ? color
      : DEFAULT_NOTE_BACKGROUND_COLOR;
  ...
  override render() {
    ...
    const currentSourceInfo = this._getCurrentSourceInfo();
    ...
    const shapeButtons = html`${ShapeComponentConfig.map(
      ({ name, generalIcon, scribbledIcon, tooltip }) => {
        return html`
          ...
          >
            ${currentSourceInfo.style === 'General'
              ? generalIcon
              : scribbledIcon}
          </edgeless-tool-icon-button>
        `;
    ...
    return html`<div class="auto-complete-panel-container" style=${style}>
      ...
      <edgeless-tool-icon-button
        ...
        .tooltip=${capitalizeFirstLetter(currentSourceInfo.name)}
        @pointerenter=${() => this._showOverlay(currentSourceInfo.name)}
        @pointerleave=${() => this._removeOverlay()}
        @click=${() => this._autoComplete(currentSourceInfo.name)}
      >
