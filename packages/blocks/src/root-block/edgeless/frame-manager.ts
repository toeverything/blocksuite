...
export class EdgelessFrameManager extends GfxExtension {
  ...
  private _watchElementAddedOrDeleted() {
    ...
    this._disposable.add(
      surfaceModel.elementAdded.on(({ id, local }) => {
        let element = surfaceModel.getElementById(id);
        if (element && local) {
    ...
    this._disposable.add(
      surfaceModel.elementRemoved.on(({ model, local }) => {
    ...
    this._disposable.add(
      doc.slots.blockUpdated.on(payload => {
        ...
        if (payload.type === 'delete') {
          if (payload.model instanceof GfxBlockModel) {
            this.removeParentFrame(payload.model);
          }
        }
      })
    );
  ...
  getElementsInFrameBound(frame: FrameBlockModel, fullyContained = true) {
    ...
    const elements: BlockSuite.EdgelessModel[] = this.gfx.grid
