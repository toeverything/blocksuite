...
> {
  ...
  private async _load() {
    ...
    this._pageMode = this._pageService.getPageMode(pageId);
    this._pageUpdatedAt = this._pageService.getPageUpdatedAt(pageId);
  ...
  open = () => {
    ...
    if (linkedDocId === this.model.page.id) {
      return;
    }
