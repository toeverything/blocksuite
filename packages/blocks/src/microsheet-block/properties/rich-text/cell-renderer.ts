...
export class RichTextCell extends BaseCellRenderer<Text> {
  static override styles = css`
    affine-microsheet-rich-text-cell {
      display: flex;
      align-items: center;
      width: 100%;
      user-select: none;
    ...
    .affine-microsheet-rich-text v-line {
    ...
    .affine-microsheet-rich-text v-line > div {
      flex-grow: 1;
    }
  `;
  ...
  get service() {
    return this.view
      .contextGet(HostContextKey)
      ?.std.getService('affine:microsheet');
  }

  get topContenteditableElement() {
    const microsheetBlock =
      this.closest<MicrosheetBlockComponent>('affine-microsheet');
    return microsheetBlock?.topContenteditableElement;
  }
  ...
  override render() {
    ...
    if (!this.value || !(this.value instanceof Text)) {
      return html`<div class="affine-microsheet-rich-text"></div>`;
    }
    return keyed(
      this.value,
      html`<rich-text
        ...
        .readonly=${true}
        class="affine-microsheet-rich-text inline-editor"
      ></rich-text>`
    );
  }
...
export class RichTextCellEditing extends BaseCellRenderer<Text> {
  static override styles = css`
    affine-microsheet-rich-text-cell-editing {
      display: flex;
      align-items: center;
      width: 100%;
      min-width: 1px;
...
declare global {
  interface HTMLElementTagNameMap {
    'affine-microsheet-rich-text-cell-editing': RichTextCellEditing;
  }
}
