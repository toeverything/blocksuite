...
export class LinkCell extends BaseCellRenderer<string> {
  static override styles = css`
    affine-microsheet-link-cell {
      width: 100%;
      user-select: none;
    }

    affine-microsheet-link-cell:hover .affine-microsheet-link-icon {
      visibility: visible;
    }

    .affine-microsheet-link {
      display: flex;
      position: relative;
      align-items: center;
      width: 100%;
    ...
    affine-microsheet-link-node {
      flex: 1;
      word-break: break-all;
    }

    .affine-microsheet-link-icon {
      position: absolute;
      right: 0;
      display: flex;
      align-items: center;
    ...
    .affine-microsheet-link-icon:hover {
      background: var(--affine-hover-color);
    }

    .affine-microsheet-link-icon svg {
      width: 16px;
      height: 16px;
      ...
      <div class="affine-microsheet-link" @click="${this._onClick}">
        ${docName
          ? html`<span
              ...
              >${docName}</span
            >`
          : html`<affine-microsheet-link-node
              .link="${linkText}"
            ></affine-microsheet-link-node>`}
        <div class="affine-microsheet-link-icon" @click="${this._onEdit}">
          ${PenIcon()}
        </div>
      </div>
    `;
...
export class LinkCellEditing extends BaseCellRenderer<string> {
  static override styles = css`
    affine-microsheet-link-cell-editing {
      width: 100%;
      cursor: text;
    }

    .affine-microsheet-link-editing {
      display: flex;
      align-items: center;
      width: 100%;
      padding: 0;
    ...
    .affine-microsheet-link-editing:focus {
      outline: none;
    }
  `;
  ...
  override render() {
    ...
    return html`<input
      class="affine-microsheet-link-editing link"
      .value=${linkText}
  ...
  @query('.affine-microsheet-link-editing')
