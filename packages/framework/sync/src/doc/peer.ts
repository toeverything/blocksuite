...
export class SyncPeer {
  ...
  handleSubdocsUpdate = ({
    ...
    for (const subdoc of added) {
      this.state.subdocsLoadQueue.push({ doc: subdoc, id: subdoc.guid });
    }
  ...
  async sync(abortOuter: AbortSignal) {
    ...
    try {
      ...
      await Promise.all([
        ...
        (async () => {
          while (throwIfAborted(abortInner.signal)) {
            const { data, id } = await this.state.pullUpdatesQueue.next(
              abortInner.signal
            );
        ...
        (async () => {
          while (throwIfAborted(abortInner.signal)) {
            const { data, id } = await this.state.pushUpdatesQueue.next(
              abortInner.signal
            );
