...
You might initially enter this page with the \`?init\` URL param. This is the default (opt-in) setup that automatically loads this built-in article. Meanwhile, you'll connect to a random single-user room via a Broadcast provider by default. This is the "single-user mode" for local testing.
...
> Note that the second and subsequent users should not open the page with the \`?init\` param in this case. Also, due to the P2P nature of Broadcast, as long as there is at least one user connected to the room, the content inside the room will **always** exist.
...
As a pro tip, you can combine multiple providers! For example, feel free to open this page with \`?providers=indexeddb,broadcast&room=hello\` params, and see if everything works as expected. Have fun!
