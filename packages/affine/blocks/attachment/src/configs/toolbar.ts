    ...
    },
{
        ...
        try {
          ...
          const blobs = await Promise.all(
            Object.entries(zipFile.files)
              .filter(([fileName, file]) => !file.dir && fileName.endsWith('.dcm'))
  ...
  const onToggle = (e: CustomEvent<boolean>) => {
    ...
    if (viewType$.value === 'Embed view' || viewType$.value === 'DICOM view') {
      console.log('Forcing reload for:', viewType$.value, 'file:', model.props.name);
      block.resourceController.updateState({ downloading: true });
      block.reload();
      block.resourceController.updateState({ downloading: false, state: 'none' });
    }
