...
export class CheckboxCell extends BaseCellRenderer<boolean> {
  static override styles = css`
    affine-microsheet-checkbox-cell {
      display: block;
      width: 100%;
      cursor: pointer;
    }

    .affine-microsheet-checkbox-container {
      height: 100%;
    }

    .affine-microsheet-checkbox {
      display: flex;
      align-items: center;
    ...
    .affine-microsheet-checkbox svg {
      width: 16px;
      height: 16px;
    }
  `;
  ...
  override render() {
    ...
    const icon = checked
      ...
      : CheckBoxUnIcon();
    return html` <div class="affine-microsheet-checkbox-container">
      <div
        class="affine-microsheet-checkbox checkbox ${checked ? 'checked' : ''}"
      >
        ${icon}
      </div>
    </div>`;
  ...
  @query('.affine-microsheet-checkbox')
