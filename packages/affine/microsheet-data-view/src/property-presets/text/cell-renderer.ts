...
export class TextCell extends BaseCellRenderer<string> {
  ...
  override render() {
    return html` <div class="affine-microsheet-text">${this.value ?? ''}</div>`;
  }
}
export class TextCellEditing extends BaseCellRenderer<string> {
  static override styles = css`
    affine-microsheet-text-cell-editing {
      display: block;
      width: 100%;
      height: 100%;
      cursor: text;
