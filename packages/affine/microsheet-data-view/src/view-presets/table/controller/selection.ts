...
export class TableSelectionController implements ReactiveController {
  ...
  get tableContainer() {
    return this.host.querySelector('.affine-microsheet-table-container');
  }
  ...
  private resolveDragStartTarget(
    target: HTMLElement
  ): [cell: MicrosheetCellContainer | null, fillValues: boolean] {
    let cell: MicrosheetCellContainer | null;
    ...
    if (fillValues) {
      ...
      cell = target.closest('affine-microsheet-cell-container');
    }
  ...
  getCellContainer(
    ...
    rowIndex: number,
    columnIndex: number
  ): MicrosheetCellContainer | undefined {
  ...
  getRect(
    ...
    const topCells = topRow.querySelectorAll(
    ...
    if (
      ...
      const max =
        (this.rows(newSelection.groupKey)
          ?.item(0)
          .querySelectorAll('affine-microsheet-cell-container').length ?? 0) -
        1;
...
export class SelectionElement extends WithDisposable(ShadowlessElement) {
  ...
  override render() {
    return html`
      <div ${ref(this.selectionRef)} class="microsheet-selection">
