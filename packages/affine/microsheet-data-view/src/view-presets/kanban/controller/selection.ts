...
export class KanbanSelectionController implements ReactiveController {
  ...
  shiftClickCard = (event: MouseEvent) => {
    ...
    const closestCardId = target.closest(
      'affine-microsheet-data-view-kanban-card'
    )?.cardId;
    const closestGroupKey = target.closest(
  ...
  focusIn() {
    ...
    if (selection.selectionType === 'card') {
      ...
      const cell = card?.querySelector(
        'affine-microsheet-data-view-kanban-cell'
      );
      if (cell) {
        this.selection = {
  ...
  focusNext(position: 'up' | 'down' | 'left' | 'right') {
    ...
    if (selection.selectionType === 'cell' && !selection.isEditing) {
      ...
      const group = this.host.querySelector(
        `affine-microsheet-data-view-kanban-group[data-key="${selection.cards[0].groupKey}"]`
      );
      const cardElements = Array.from(
        group?.querySelectorAll('affine-microsheet-data-view-kanban-card') ?? []
      );
  ...
  getNextFocusCard(
    ...
    const groups = Array.from(
      this.host.querySelectorAll('affine-microsheet-data-view-kanban-group')
    );
    ...
    throw new BlockSuiteError(
      ErrorCode.MicrosheetBlockError,
  ...
  getNextFocusCell(
    ...
    const kanbanCells = getCardCellsBySelection(this.host, selection);
    const group = this.host.querySelector(
      `affine-microsheet-data-view-kanban-group[data-key="${selection.groupKey}"]`
    );
...
function getNextGroupFocusElement(
  ...
  const nextCards = Array.from(
    nextGroup.querySelectorAll('affine-microsheet-data-view-kanban-card')
  );
  ...
  const cells = Array.from(
    nextCard.querySelectorAll('affine-microsheet-data-view-kanban-cell')
  );
  const cellPos = cells
    .map((card, index) => {
      const targetRect = card.getBoundingClientRect();
...
function getCardCellsBySelection(
  ...
  return Array.from(
    card?.querySelectorAll('affine-microsheet-data-view-kanban-cell') ?? []
  );
}
...
function getSelectedCard(
  ...
  return group.querySelector<KanbanCard>(
    `affine-microsheet-data-view-kanban-card[data-card-id="${selection.cardId}"]`
  );
}
  ...
  const groups = groupKeys
    .map(key =>
      viewElement.querySelector(
        `affine-microsheet-data-view-kanban-group[data-key="${key}"]`
      )
    )
    ...
    .flatMap(group =>
      cardIds.map(id =>
        group.querySelector<KanbanCard>(
          `affine-microsheet-data-view-kanban-card[data-card-id="${id}"]`
        )
      )
    )
...
function getFocusCell(viewElement: Element, selection: KanbanCellSelection) {
  ...
  return card?.querySelector<KanbanCell>(
    `affine-microsheet-data-view-kanban-cell[data-column-id="${selection.columnId}"]`
  );
}
