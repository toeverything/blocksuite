...
const styles = css`
  .affine-microsheet-toolbar-item.new-record {
    margin-left: 12px;
    display: flex;
    align-items: center;
    gap: 4px;
...
export class DataViewHeaderToolsAddRow extends WidgetBase {
  ...
  _dragStart = (e: MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    const container = this.closest('affine-microsheet-data-view-renderer');
    const tableRect = container
      ?.querySelector('affine-microsheet-table')
      ?.getBoundingClientRect();
    const rows: NodeListOf<HTMLElement> | undefined =
      container?.querySelectorAll('.affine-microsheet-block-row');
    if (!rows || !tableRect) {
      return;
    }
  ...
  override render() {
    ...
    return html` <div
      class="affine-microsheet-toolbar-item new-record"
      draggable="true"
