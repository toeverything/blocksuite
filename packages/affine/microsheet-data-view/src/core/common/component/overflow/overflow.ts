...
export class Overflow extends SignalWatcher(WithDisposable(ShadowlessElement)) {
  static override styles = css`
    ...
    .microsheet-component-overflow-item {
    }
    .microsheet-component-overflow-item.hidden {
      opacity: 0;
      pointer-events: none;
      position: absolute;
    }
    ...
    return html`
      ${repeat(this.renderItem, (render, index) => {
        const className = classMap({
          'microsheet-component-overflow-item': true,
          hidden: index >= this.renderCount,
        });
        return html`<div class="${className}">${render()}</div>`;
      })}
      <div class="microsheet-component-overflow-more">
  ...
  @queryAll(':scope > .microsheet-component-overflow-item')
  accessor items!: HTMLDivElement[] & NodeList;

  @query(':scope > .microsheet-component-overflow-more')
