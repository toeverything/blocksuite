...
export class ChatPanelInput extends WithDisposable(LitElement) {
  ...
  send = async () => {
    const { markdown, status } = this.chatContextValue;
    ...
    this.updateContext({
      items: [
        ...
        { content: '', createdAt: new Date().toISOString(), role: 'assistant' },
      ],
    });
    ...
    try {
      ...
      this.updateContext({ error: error as AIError, status: 'error' });
    } finally {
        ...
        ${this.chatContextValue.quote
          ? html`<div class="chat-selection-quote">
              ...
              <div
                ...
                @click=${() => {
                  this.updateContext({ markdown: '', quote: '' });
                }}
              >
                ${CloseIcon}
              </div>
